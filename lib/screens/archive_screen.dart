        ...
        builder: (context, snapshot) {
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return ArchivedLocationCard(location: location);
            },
          );
        });
  }
...
class _ArchivedLocationCardState extends State<ArchivedLocationCard> {
  ...
  Widget build(BuildContext context) {
    return Card(
      ...
      child: ListTile(
        ...
        title: Text(widget.location.partieNr),
        subtitle: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('Normal: ${widget.location.normalQuantity} fm'),
            Text('ÜS: ${widget.location.oversizeQuantity} fm'),
          ],
        ),
        trailing: Row(
  ...
  Widget _buildLetterAvatar(BuildContext context) {
    return Container(
      ...
      child: Text(
        widget.location.sawmill!.isNotEmpty ? widget.location.sawmill![0].toUpperCase() : '?',
        style: TextStyle(
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            onPressed: () {
              context.read<DataProvider>().deleteLocation(widget.location.id);
    ...
    return Consumer<DataProvider>(
      builder: (context, provider, child) {
        ...
        return RefreshIndicator(
          onRefresh: () async {
            await provider.loadArchivedLocations();
          ...
          if (_isExpanded)
            FutureBuilder<List<Shipment>>(
              future: Future.value(context
                      .read<DataProvider>()
                      .shipmentsByLocation[widget.location.id] ??
                  []),
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  return const Center(child: CircularProgressIndicator());
